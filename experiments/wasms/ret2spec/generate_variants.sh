...
if [[ ! -f stacking.${VERSION}.gz ]]; then
    if [[ "$(uname -s)" == "Darwin" ]]; then
        curl --fail --location --silent https://github.com/Jacarte/tawasco/releases/download/${VERSION}/stacking-all-x86_64-macos-${VERSION}.gz --output stacking.${VERSION}.gz || exit 1
...
do
    ...
    RUST_LOG=wasm_mutate=debug ./stacking ret2spec_sa.wasm ret2_pool/ret2.stack.$S.wasm --seed $S -c 1000 --save 1  2> ret2_pool/logs.$S.break.txt
